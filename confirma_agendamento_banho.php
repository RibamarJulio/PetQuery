...
require('conexao.php');
$serv = filter_input(INPUT_POST,'serv_banho',FILTER_SANITIZE_STRING);
$idpet = $_SESSION['id'];
if($serv == 'Banho'){
    $valor = 110.00;
} else if ($serv == 'Banho e tosa higiênica'){
    $valor = 125.00;
} else if ($serv == 'Banho e hidratação'){
    $valor = 150.00;
} else if ($serv == 'Banho e tosa máquina'){
    $valor = 155.00;
} else if ($serv == 'Banho e tosa tesoura'){
    $valor = 200.00;
} else if ($serv == 'Consulta clínica'){
    $valor = 180.00;
        ...
        <div class="container-fluid" style="background-color: #D4E1F2">
            <div class="row p-4">
                ...
                <div class="col-6 text-start">
                    <span><img id="relogio" src="img/icons/icons_serviçosPet/relogio.png" alt="icone de relogio" style="width: 35px; margin-right: 10px; margin-left: 35px"><small id="hora" style="font-weight: 600; color: #464444; font-size: 16px;"><input id="time" class="p-1" style="border: 1px solid #a9a9a9; border-radius: 5px" min="07:00:00" max="20:00:00" type="time" name="time"></small></span>
                </div>
            </div>
        </div>

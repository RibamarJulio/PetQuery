function func(){
    document.getElementById('list_func').style.display = 'block';
...
function cli(){
    document.getElementById('list_func').style.display = 'none';
...
function searchfunc(){
    let filter = document.getElementById('searchfunc').value.toUpperCase();
    ...
    for (let i = 0; i <= l.length; i++){
        let a=func[i].getElementsByTagName('h5')[0];

        let value = a.innerHTML || a.innerText || a.textContent;

        if(value.toUpperCase().indexOf(filter) > -1){
            func[i].style.display="";
            document.getElementById('noresultfunc').innerText = "";
        } else {
            func[i].style.display="none";
            document.getElementById('noresultfunc').innerText = "Nenhum resultado foi encontrado.";
        }
    }
}
